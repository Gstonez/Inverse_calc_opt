...
class GeneticAlgorithm:
    ...
    def site_cal(self):
        """calculate the site bock of individual"""
        for i in range(self.sizepop):
            site_x = self.population[i].chrom[0] // ((self.bound[0][1] - self.bound[0][0]) / 5)
            site_y = (self.population[i].chrom[1] - self.bound[1][0]) //  ((self.bound[1][1] - self.bound[1][0]) / 5)
            site_z = self.population[i].chrom[2] // ((self.bound[2][1] - self.bound[2][0]) / 8)
            site = site_x + site_z * 25 + site_y * 5
            self.population[i].site = int(site)
            self.bad_block_num.append(site)
        self.bad_block_num = list(set(self.bad_block_num))
    ...
    def solve(self):
        ...
        while (self.t < self.MAXGEN - 1):
            ...
            F1 = 0.9 - ((self.MAXGEN - self.t + 1) / self.MAXGEN) * 0.5
            if self.selec_num == 1:
                ...
                self.tournament_selection_operation(n=tournament_n, m=tournament_m)  # 锦标赛选择
            if self.cross_num == 1:
                ...
                self.differential_evolution_operation(option=2, F1=F1, CR=cross_rate)  # DE/current-to-best/1
            elif self.cross_num == 5:
                self.differential_evolution_operation(option=3, CR=cross_rate)  # DE/best/1
            ...
            if self.t == 9:
                if (self.trace[int(self.t), 1] < 1e-3) & (self.trace[int(self.t), 0] < 1e-2):
                    self.site_cal()
                    block_num = np.arange(self.sizepop)
                    block_num = np.delete(block_num, self.bad_block_num)
                    for i in range(2):
                        self.population[i] = self.best
                    for i in range(2, self.sizepop):
                        site = np.random.choice(block_num)
                        x_block_num = (site % 25) % 5  # 第一维上的分块序号
                        x_low = self.bound[0][0] + ((self.bound[0][1] - self.bound[0][0]) / 5) * x_block_num
                        x_high = self.bound[0][0] + ((self.bound[0][1] - self.bound[0][0]) / 5) * (x_block_num + 1)
                        self.population[i].chrom[0] = np.random.uniform(x_low, x_high)
                        y_block_num = (site % 25) // 5  # 第二维上的分块序号
                        y_low = self.bound[1][0] + ((self.bound[1][1] - self.bound[1][0]) / 5) * y_block_num
                        y_high = self.bound[1][0] + ((self.bound[1][1] - self.bound[1][0]) / 5) * (y_block_num + 1)
                        self.population[i].chrom[1] = np.random.uniform(y_low, y_high)
                        z_block_num = site // 25  # 第三维上的分块序号
                        z_low = self.bound[2][0] + ((self.bound[2][1] - self.bound[2][0]) / 4) * z_block_num
                        z_high = self.bound[2][0] + ((self.bound[2][1] - self.bound[2][0]) / 4) * (z_block_num + 1)
                        self.population[i].chrom[2] = np.random.uniform(z_low, z_high)
                    self.use_count += 1
    ...
    def tournament_selection_operation(self, n=10, m=3):
        """
        this function select the next generation based on the
        propotion of each individual
        ------------------------------------------------------
    ...
    def differential_evolution_operation(self, F=0.5, F1=0.5, CR=1, option=1):
        """
        ...
        for i in range(0, self.sizepop):
            ...
            if u <= CR:  # 判定是否进行交叉
                ...
                for j in range(cross_pos, self.vardim):
                    ...
                    if option == 2:
                        y.chrom[j] = y.chrom[j] + F1 * (self.best.chrom[j] -
                                                        parents[0].chrom[j])
                            ...
                            for size in popsize:  # 遍历种群规模
                                for gen in maxgen:  # 遍历迭代次数
                                    ...
                                    # for i in range(5): 
                                        ...
                                        result = demo.solve()
                                        if demo.trace[-1, 0] > 1:
                                            use_count = use_count + demo.use_count
                                        # for i in range(size):
                                            # print(demo.population[i].chrom)
                                        path = get_path(r"tmp1\detials\ " + str(size) + "_" + str(gen) + "_" +
                                                        str(c_n) + "_" + str(s_n) + "_" +
                                                        "adapt_F=" + str(f) + "_" + str(c_f) + "_" + str(n) +
                                                        "_" + str(m) + "_" +  str(i+1) + "_" +  "best_trace.txt")
                                        np.savetxt(path, demo.trace[:, 0])
                                        path = get_path(r"tmp1\detials\ " + str(size) + "_" + str(gen) + "_" +
                                                        str(c_n) + "_" + str(s_n) + "_" +
                                                        "adapt_F=" + str(f) + "_" + str(c_f) + "_" + str(n) +
                                                        "_" + str(m) + "_" +  str(i+1) + "_" +  "aver_trace.txt")
                                        np.savetxt(path, demo.trace[:, 1])
                                        final_result.append([demo.best.chrom[0],
                                                             demo.best.chrom[1],
                                                             demo.best.chrom[2],
                                                             demo.trace[-1, 0]])
                                    ...
                                    np.savetxt(get_path("tmp1\\ " + str(size) + "_" + str(gen) + "_" +
                                                        str(c_n) + "_" + str(s_n) + "_" + "adapt_F=" + str(f) +
                                                        "_" + str(c_f) + "_" + str(n) + "_" + str(m) + "_" + "all_result.txt"), final_result)
                                    ...
                                    np.savetxt(get_path("tmp1\\ " + str(size) + "_" + str(gen) + "_" +
                                                        str(c_n) + "_" + str(s_n) + "_" + "adapt_F=" + str(f) +
                                                        "_" + str(c_f) + "_" + str(n) + "_" + str(m) + "_" + "result.txt"), [minmum, maxmum, aver, std])
                                    print('--------')
                                    print(use_count)
                                    print('--------')
                                    np.savetxt(get_path("tmp1\\ " + str(size) + "_" + str(gen) + "_" +
                                                        str(c_n) + "_" + str(s_n) + "_" + "adapt_F=" + str(f) +
                                                        "_" + str(c_f) + "_" + str(n) + "_" + str(m) + "_" + "useful.txt"), [use_count])
